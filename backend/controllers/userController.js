    ...
    try {
        ...
        if (user) {
            ...
            if (isMatching) {
                ...
                console.log({ token });
                return res.status(200).cookie("jwt", token, { httpOnly: true, sameSite: "None", secure: true, domain: "https://backend-q7jk.onrender.com/api/user/login" }).json({ msg: "Login erfolgreich!" })
            }
